...
import com.nexa.cityseason.util.MyApp;
...
public class NavigationAdapter extends BaseAdapter {
	...
	public void setDownloadsCounter(int count){
		...
		mList.get(Constant.MENU_DASHBOARD).counter = mDownloadsCounter;
		notifyDataSetChanged();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		ViewHolder holder = null;
		  Typeface custom_font = Typeface.createFromAsset(mcontext.getAssets(), "fonts/GothamRnd-Light.otf");
		if (convertView == null) {
		...
		if (!item.isHeader) {			
			if (item.checked) {
				...
				String theme= MyApp.getInstance().getSetting().getString("themecolor", "");
				if(theme.equalsIgnoreCase("yellow")){
					
					holder.viewNavigation.setBackgroundColor(mcontext.getResources().getColor(R.color.yellow_light));
				
				}
				else if(theme.equalsIgnoreCase("green")){
					
					holder.viewNavigation.setBackgroundColor(mcontext.getResources().getColor(R.color.greenspices_light));
			
				}
				else{
				
					holder.viewNavigation.setBackgroundColor(mcontext.getResources().getColor(R.color.dashboard_actionbar));
